...
{
	public class FI_CustLedgerFactory:Common.Business.FI_CustLedger
    {
        public static Common.Business.FI_CustLedger Fetch(FI_CustLedger data)
        {
            Common.Business.FI_CustLedger item = (Common.Business.FI_CustLedger)Activator.CreateInstance(typeof(Common.Business.FI_CustLedger));
		...
		private void DataPortal_Fetch(Common.Business.LambdaExpression lambda)
        {
            ...
            {
				var exp = lambda.Resolve<FI_CustLedger>();
                var i = (from p in ctx.DataContext.FI_CustLedger.Where(exp)
	...
	public class FI_CustLedgersFactory : Common.Business.FI_CustLedgers
    {
        private void DataPortal_Fetch()
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var i = from p in ctx.DataContext.FI_CustLedger
                        select FI_CustLedgerFactory.Fetch(p);
		...
		private void DataPortal_Fetch(Paging page)
        {
            ...
            {
                var i = (from p in ctx.DataContext.FI_CustLedger
                        select FI_CustLedgerFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
                this.AddRange(i);
                this.IsReadOnly = true;
        ...
        private void DataPortal_Fetch(PagigExpress page)
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var exp = page.Lambda.Resolve<FI_CustLedger>();
                var i = (from p in ctx.DataContext.FI_CustLedger.Where(exp)
                         select FI_CustLedgerFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
