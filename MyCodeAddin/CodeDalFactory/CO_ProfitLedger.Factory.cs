...
{
	public class CO_ProfitLedgerFactory:Common.Business.CO_ProfitLedger
    {
        public static Common.Business.CO_ProfitLedger Fetch(CO_ProfitLedger data)
        {
            Common.Business.CO_ProfitLedger item = (Common.Business.CO_ProfitLedger)Activator.CreateInstance(typeof(Common.Business.CO_ProfitLedger));
		...
		private void DataPortal_Fetch(Common.Business.LambdaExpression lambda)
        {
            ...
            {
				var exp = lambda.Resolve<CO_ProfitLedger>();
                var i = (from p in ctx.DataContext.CO_ProfitLedger.Where(exp)
	...
	public class CO_ProfitLedgersFactory : Common.Business.CO_ProfitLedgers
    {
        private void DataPortal_Fetch()
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var i = from p in ctx.DataContext.CO_ProfitLedger
                        select CO_ProfitLedgerFactory.Fetch(p);
		...
		private void DataPortal_Fetch(Paging page)
        {
            ...
            {
                var i = (from p in ctx.DataContext.CO_ProfitLedger
                        select CO_ProfitLedgerFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
                this.AddRange(i);
                this.IsReadOnly = true;
        ...
        private void DataPortal_Fetch(PagigExpress page)
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var exp = page.Lambda.Resolve<CO_ProfitLedger>();
                var i = (from p in ctx.DataContext.CO_ProfitLedger.Where(exp)
                         select CO_ProfitLedgerFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
