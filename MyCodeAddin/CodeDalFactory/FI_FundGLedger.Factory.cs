...
{
	public class FI_FundGLedgerFactory:Common.Business.FI_FundGLedger
    {
        public static Common.Business.FI_FundGLedger Fetch(FI_FundGLedger data)
        {
            Common.Business.FI_FundGLedger item = (Common.Business.FI_FundGLedger)Activator.CreateInstance(typeof(Common.Business.FI_FundGLedger));
		...
		private void DataPortal_Fetch(Common.Business.LambdaExpression lambda)
        {
            ...
            {
				var exp = lambda.Resolve<FI_FundGLedger>();
                var i = (from p in ctx.DataContext.FI_FundGLedger.Where(exp)
	...
	public class FI_FundGLedgersFactory : Common.Business.FI_FundGLedgers
    {
        private void DataPortal_Fetch()
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var i = from p in ctx.DataContext.FI_FundGLedger
                        select FI_FundGLedgerFactory.Fetch(p);
		...
		private void DataPortal_Fetch(Paging page)
        {
            ...
            {
                var i = (from p in ctx.DataContext.FI_FundGLedger
                        select FI_FundGLedgerFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
                this.AddRange(i);
                this.IsReadOnly = true;
        ...
        private void DataPortal_Fetch(PagigExpress page)
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var exp = page.Lambda.Resolve<FI_FundGLedger>();
                var i = (from p in ctx.DataContext.FI_FundGLedger.Where(exp)
                         select FI_FundGLedgerFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
