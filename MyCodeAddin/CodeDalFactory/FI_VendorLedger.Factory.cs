...
{
	public class FI_VendorLedgerFactory:Common.Business.FI_VendorLedger
    {
        public static Common.Business.FI_VendorLedger Fetch(FI_VendorLedger data)
        {
            Common.Business.FI_VendorLedger item = (Common.Business.FI_VendorLedger)Activator.CreateInstance(typeof(Common.Business.FI_VendorLedger));
		...
		private void DataPortal_Fetch(Common.Business.LambdaExpression lambda)
        {
            ...
            {
				var exp = lambda.Resolve<FI_VendorLedger>();
                var i = (from p in ctx.DataContext.FI_VendorLedger.Where(exp)
	...
	public class FI_VendorLedgersFactory : Common.Business.FI_VendorLedgers
    {
        private void DataPortal_Fetch()
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var i = from p in ctx.DataContext.FI_VendorLedger
                        select FI_VendorLedgerFactory.Fetch(p);
		...
		private void DataPortal_Fetch(Paging page)
        {
            ...
            {
                var i = (from p in ctx.DataContext.FI_VendorLedger
                        select FI_VendorLedgerFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
                this.AddRange(i);
                this.IsReadOnly = true;
        ...
        private void DataPortal_Fetch(PagigExpress page)
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var exp = page.Lambda.Resolve<FI_VendorLedger>();
                var i = (from p in ctx.DataContext.FI_VendorLedger.Where(exp)
                         select FI_VendorLedgerFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
