...
{
	public class FI_OpenVoucherFactory:Common.Business.FI_OpenVoucher
    {
        public static Common.Business.FI_OpenVoucher Fetch(FI_OpenVoucher data)
        {
            Common.Business.FI_OpenVoucher item = (Common.Business.FI_OpenVoucher)Activator.CreateInstance(typeof(Common.Business.FI_OpenVoucher));
		...
		private void DataPortal_Fetch(Common.Business.LambdaExpression lambda)
        {
            ...
            {
				var exp = lambda.Resolve<FI_OpenVoucher>();
                var i = (from p in ctx.DataContext.FI_OpenVoucher.Where(exp)
	...
	public class FI_OpenVouchersFactory : Common.Business.FI_OpenVouchers
    {
        private void DataPortal_Fetch()
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var i = from p in ctx.DataContext.FI_OpenVoucher
                        select FI_OpenVoucherFactory.Fetch(p);
		...
		private void DataPortal_Fetch(Paging page)
        {
            ...
            {
                var i = (from p in ctx.DataContext.FI_OpenVoucher
                        select FI_OpenVoucherFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
                this.AddRange(i);
                this.IsReadOnly = true;
        ...
        private void DataPortal_Fetch(PagigExpress page)
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var exp = page.Lambda.Resolve<FI_OpenVoucher>();
                var i = (from p in ctx.DataContext.FI_OpenVoucher.Where(exp)
                         select FI_OpenVoucherFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
