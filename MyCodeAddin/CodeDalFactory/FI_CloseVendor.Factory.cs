...
{
	public class FI_CloseVendorFactory:Common.Business.FI_CloseVendor
    {
        public static Common.Business.FI_CloseVendor Fetch(FI_CloseVendor data)
        {
            Common.Business.FI_CloseVendor item = (Common.Business.FI_CloseVendor)Activator.CreateInstance(typeof(Common.Business.FI_CloseVendor));
		...
		private void DataPortal_Fetch(Common.Business.LambdaExpression lambda)
        {
            ...
            {
				var exp = lambda.Resolve<FI_CloseVendor>();
                var i = (from p in ctx.DataContext.FI_CloseVendor.Where(exp)
	...
	public class FI_CloseVendorsFactory : Common.Business.FI_CloseVendors
    {
        private void DataPortal_Fetch()
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var i = from p in ctx.DataContext.FI_CloseVendor
                        select FI_CloseVendorFactory.Fetch(p);
		...
		private void DataPortal_Fetch(Paging page)
        {
            ...
            {
                var i = (from p in ctx.DataContext.FI_CloseVendor
                        select FI_CloseVendorFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
                this.AddRange(i);
                this.IsReadOnly = true;
        ...
        private void DataPortal_Fetch(PagigExpress page)
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var exp = page.Lambda.Resolve<FI_CloseVendor>();
                var i = (from p in ctx.DataContext.FI_CloseVendor.Where(exp)
                         select FI_CloseVendorFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
