...
{
	public class FI_OpenVendorFactory:Common.Business.FI_OpenVendor
    {
        public static Common.Business.FI_OpenVendor Fetch(FI_OpenVendor data)
        {
            Common.Business.FI_OpenVendor item = (Common.Business.FI_OpenVendor)Activator.CreateInstance(typeof(Common.Business.FI_OpenVendor));
		...
		private void DataPortal_Fetch(Common.Business.LambdaExpression lambda)
        {
            ...
            {
				var exp = lambda.Resolve<FI_OpenVendor>();
                var i = (from p in ctx.DataContext.FI_OpenVendor.Where(exp)
	...
	public class FI_OpenVendorsFactory : Common.Business.FI_OpenVendors
    {
        private void DataPortal_Fetch()
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var i = from p in ctx.DataContext.FI_OpenVendor
                        select FI_OpenVendorFactory.Fetch(p);
		...
		private void DataPortal_Fetch(Paging page)
        {
            ...
            {
                var i = (from p in ctx.DataContext.FI_OpenVendor
                        select FI_OpenVendorFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
                this.AddRange(i);
                this.IsReadOnly = true;
        ...
        private void DataPortal_Fetch(PagigExpress page)
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var exp = page.Lambda.Resolve<FI_OpenVendor>();
                var i = (from p in ctx.DataContext.FI_OpenVendor.Where(exp)
                         select FI_OpenVendorFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
