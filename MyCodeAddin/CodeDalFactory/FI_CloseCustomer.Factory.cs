...
{
	public class FI_CloseCustomerFactory:Common.Business.FI_CloseCustomer
    {
        public static Common.Business.FI_CloseCustomer Fetch(FI_CloseCustomer data)
        {
            Common.Business.FI_CloseCustomer item = (Common.Business.FI_CloseCustomer)Activator.CreateInstance(typeof(Common.Business.FI_CloseCustomer));
		...
		private void DataPortal_Fetch(Common.Business.LambdaExpression lambda)
        {
            ...
            {
				var exp = lambda.Resolve<FI_CloseCustomer>();
                var i = (from p in ctx.DataContext.FI_CloseCustomer.Where(exp)
	...
	public class FI_CloseCustomersFactory : Common.Business.FI_CloseCustomers
    {
        private void DataPortal_Fetch()
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var i = from p in ctx.DataContext.FI_CloseCustomer
                        select FI_CloseCustomerFactory.Fetch(p);
		...
		private void DataPortal_Fetch(Paging page)
        {
            ...
            {
                var i = (from p in ctx.DataContext.FI_CloseCustomer
                        select FI_CloseCustomerFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
                this.AddRange(i);
                this.IsReadOnly = true;
        ...
        private void DataPortal_Fetch(PagigExpress page)
        {
            using (var ctx = ContextManager<EFDataContext>.GetManager(Database.EFramework))
            {
                var exp = page.Lambda.Resolve<FI_CloseCustomer>();
                var i = (from p in ctx.DataContext.FI_CloseCustomer.Where(exp)
                         select FI_CloseCustomerFactory.Fetch(p)).Skip(page.StartIndex).Take(page.RowCount);
                this.RaiseListChangedEvents = false;
                this.IsReadOnly = false;
